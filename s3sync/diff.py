...
class Diff(object):
	...
	def generate_sync_list(self):
		...
		for name, obj in to_sync.items():
			if self.source_type == "s3" and self.destination_type == "s3":
				...
				# this is not ideal
				source_file = "s3://" + os.path.sep.join([
					self.source_bucket,
					obj["key"],
				])
				item = self.get_list_item(source_file)
			...
			if "md5sum" in obj: item["md5sum"] = obj["md5sum"]
			if "size" in obj: item["size"] = obj["size"]
	...
	def get_s3_files(self, bucket=None, path=None):
		output = {}
		try:
			prefix = "{}/".format(path) if path else ""
			resp = self.s3.list_objects_v2(Bucket=bucket, Prefix=prefix, MaxKeys=10000)
		except:
